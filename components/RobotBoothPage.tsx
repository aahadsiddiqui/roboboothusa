...
export default function RobotBoothPage({ market = MARKETS.national }: { market?: MarketConfig }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append('source', market.id === 'national' ? 'Robot Photobooth Page' : `Robot Photobooth Page (${market.analyticsRegion})`)
        ...
        <div className="min-h-screen bg-black text-white overflow-x-hidden">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">
                <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-white/60 text-xs font-medium">{rc.ratingSubtext}</span>
                  </div>
          ...
          <section className="py-4 md:py-6 border-y border-[#fce4a6]/10 overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 mb-3">
              <p className="text-center text-[#fce4a6]/60 text-[10px] md:text-xs font-semibold tracking-[0.2em] uppercase">{rc.trustedCompanies}</p>
            </div>
          ...
          <section className="py-10 md:py-14 px-4 border-t border-white/5">
            <Reveal className="max-w-3xl mx-auto text-center">
              <h2 className="text-xl md:text-2xl lg:text-4xl font-black mb-2 md:mb-3">
                Your Event Deserves <span className="text-[#fce4a6]">{rc.finalCtaTitle}</span>
              </h2>
...
const testimonials = [
  { name: 'Sarah J.', role: 'Wedding Host, Chicago', text: 'The robot was the highlight of our wedding. Guests couldn\'t stop talking about it and the photos were incredible. It roamed around and captured moments we would have missed.' },
