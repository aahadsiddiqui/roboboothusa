...
import { FiArrowRight, FiCheck, FiPhone, FiChevronDown, FiChevronUp, FiClock, FiX, FiCamera, FiZap, FiImage, FiUsers, FiStar, FiShield, FiPrinter, FiSmile } from 'react-icons/fi'
...
/* ════════════════════════════════════════════════════════════════
   PREMIUM PHOTOBOOTH — Custom Landing Page (mirroring Robot & Aerial layout)
   ════════════════════════════════════════════════════════════════ */
export default function PremiumPhotoboothPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append('source', market.id === 'national' ? 'Premium Photobooth Page' : `Premium Photobooth Page (${market.analyticsRegion})`)
  ...
  return (
    <>
      <Head>
        <title>
          {market.id === 'national'
            ? 'Premium Photobooth Rental Chicago USA | Sleek Enclosed Photo Booth | Robo Booth'
            : `Premium Photobooth | Robo Booth ${market.analyticsRegion}`}
        </title>
        <meta
          name="description"
          content={
        ...
        />
        <meta name="keywords" content="premium photobooth rental Chicago, enclosed photo booth, photo booth USA, DSLR photo booth, instant print photo booth, wedding photo booth Chicago, corporate photo booth, portable photo booth" />
        <meta
          property="og:title"
          content={market.id === 'national' ? 'Premium Photobooth Rental Chicago USA | Robo Booth' : `Premium Photobooth | Robo Booth ${market.analyticsRegion}`}
        />
        <meta
          property="og:description"
          content={
            market.id === 'national'
              ? 'Sleek, portable photo booth with DSLR-quality photos, videos & GIFs. Instant sharing and prints. Chicago & USA.'
              : 'Sleek, portable photo booth with DSLR-quality photos, videos & GIFs. Instant sharing and prints.'
          }
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={ogUrl} />
        <link rel="canonical" href={canonical} />
        <link rel="preload" href="/images/premium-booth.jpg" as="image" />
      ...
      <div className={showModal ? 'blur-sm pointer-events-none select-none' : ''}>
        <div className="min-h-screen bg-black text-white overflow-x-hidden">
          ...
          {/* ═══════════════════════════════════════
              HERO — Single large image (no video)
             ═══════════════════════════════════════ */}
          <section className={`relative ${urgencyDismissed ? 'pt-20 md:pt-24' : 'pt-[7rem] md:pt-[8rem]'} pb-6 md:pb-8 px-4`}>
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">
                <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-white/60 text-xs font-medium">
                      5.0 Rating ·{' '}
                      {market.id === 'chicago' ? "Chicago" : market.id === 'texas' ? "Texas" : "USA"}&apos;s Premium Photo Booth
                    </span>
                  </div>
                  <h1 className="text-[1.65rem] leading-[1.15] md:text-4xl lg:text-5xl font-black md:leading-[1.1] mb-4">
                    The Premium Photobooth That{' '}
                    <span className="text-[#fce4a6]">Elevates Every Event.</span>
                  </h1>
                  <p className="text-white/80 text-sm md:text-base lg:text-lg leading-relaxed mb-5 max-w-xl">
                    Sleek, portable, and packed with studio-grade lighting. DSLR-quality photos, videos, and GIFs with instant sharing and on-site prints.{' '}
                    <span className="text-white font-semibold">We handle setup, operation, and everything in between.</span>
                  </p>
                ...
                {/* Hero image — premium-booth.jpg */}
                <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.15 }} className="hidden md:block">
                  <img src="/images/premium-booth.jpg" alt="Premium Photobooth" className="w-full h-[480px] lg:h-[520px] object-cover rounded-2xl shadow-2xl" loading="eager" fetchPriority="high" />
                </motion.div>

                {/* Mobile hero */}
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="md:hidden -mx-4">
                  <img src="/images/premium-booth.jpg" alt="Premium Photobooth" className="w-full h-60 object-cover" loading="eager" fetchPriority="high" />
                </motion.div>
              </div>
            </div>
          </section>
          ...
          <section className="py-8 md:py-10 px-4">
            <div className="max-w-5xl mx-auto">
              <Reveal className="text-center mb-5">
                <h2 className="text-xl md:text-2xl lg:text-3xl font-black mb-1.5">See the Premium Photobooth <span className="text-[#fce4a6]">in Action</span></h2>
                <p className="text-white/50 text-xs md:text-sm">Watch how guests experience it at real events across Chicago</p>
              </Reveal>
              <Reveal delay={0.1}>
                <div className="rounded-2xl overflow-hidden border border-white/10 bg-black flex justify-center">
                  <video className="w-full h-auto max-h-[70vh]" autoPlay loop muted playsInline controls={false} preload="auto" disablePictureInPicture style={{ display: 'block' }}>
                    ...
                    <source src="/videos/premiumphoto.mov" type="video/mp4" />
                  </video>
                </div>
              </Reveal>
            </div>
          ...
          <section className="py-8 md:py-10 px-4 border-t border-white/5">
            <div className="max-w-5xl mx-auto">
              <Reveal className="text-center mb-6">
                ...
                <p className="text-white/50 text-xs md:text-sm">Five reasons the Premium Photobooth is a crowd favorite</p>
              </Reveal>
          ...
          <section className="py-10 md:py-14 px-4 border-t border-white/5">
            <Reveal className="max-w-3xl mx-auto text-center">
              <h2 className="text-xl md:text-2xl lg:text-4xl font-black mb-2 md:mb-3">
                Your Event Deserves the <span className="text-[#fce4a6]">Premium Photobooth Experience.</span>
              </h2>
              <p className="text-white/60 text-xs md:text-sm lg:text-base mb-5 max-w-lg mx-auto">
                Give your guests an experience they&apos;ll be talking about long after the event is over.
              </p>
...
const howItWorks = [
  ...
  { title: 'We Show Up', desc: 'Our team arrives early, sets up the Premium Photobooth in minutes with studio lighting and props, and tests everything before guests arrive.' },
  { title: 'Your Guests Are Wow\'d', desc: 'Guests step in, strike a pose, and walk away with instant prints and digital copies they can share anywhere.' },
]

const whyGuestsLove = [
  { icon: <FiCamera className="w-5 h-5 md:w-6 md:h-6" />, title: 'DSLR-Quality Photos', desc: 'Studio-grade camera and lighting deliver professional results every time — no blurry phone photos, just flawless shots.' },
  ...
  { icon: <FiPrinter className="w-5 h-5 md:w-6 md:h-6" />, title: 'Premium On-Site Prints', desc: 'High-quality prints on premium paper stock that guests take home as a tangible keepsake from your event.' },
  { icon: <FiSmile className="w-5 h-5 md:w-6 md:h-6" />, title: 'Sleek & Compact Design', desc: 'Modern, portable design that fits into any venue — from intimate spaces to grand ballrooms. Looks premium everywhere.' },
  { icon: <FiUsers className="w-5 h-5 md:w-6 md:h-6" />, title: 'Dedicated On-Site Attendant', desc: 'A professional attendant manages the entire experience — guiding guests, keeping the line moving, and ensuring flawless operation.' },
]

const customizations = [
  { icon: <FiStar className="w-5 h-5" />, title: 'Custom Backdrops', desc: 'Choose from our premium backdrop collection or we\'ll create a custom one that matches your event theme, brand, or color palette perfectly.' },
  { icon: <FiImage className="w-5 h-5" />, title: 'Branded Overlays & Filters', desc: 'Custom photo overlays with your logo, event name, date, hashtag, and brand colors. Every photo becomes a branded keepsake your guests will share.' },
  { icon: <FiShield className="w-5 h-5" />, title: 'Full White-Glove Service', desc: 'We handle every detail — delivery, setup, operation, and teardown. You don\'t lift a finger. Just enjoy the event and take the credit.' },
]

const testimonials = [
  { name: 'Matthew K.', role: 'Event Organizer, Naperville', text: 'The Premium Photobooth was compact but looked absolutely premium. Guests were blown away by the photo quality and the instant prints were a huge hit. Would definitely book again.' },
  { name: 'Garrett V.', role: 'Corporate Host, Chicago', text: 'Setup was incredibly fast and the sharing options made it so easy for our team. The branded overlays tied everything together beautifully. Best vendor decision we made.' },
  { name: 'Priya S.', role: 'Wedding Planner', text: 'Fit perfectly in our venue and the photos were stunning. The attendant was professional, the line moved quickly, and every guest left with a keepsake. Highly recommend.' },
]

const faqs = [
  { question: 'How much space does the Premium Photobooth need?', answer: 'We typically need a 6x6 ft area with access to a standard power outlet. The booth is designed to fit into tight spaces without sacrificing quality — perfect for any venue size.' },
  { question: 'What\'s included in the rental?', answer: 'Everything — the full photobooth setup, DSLR camera, studio lighting, backdrop, on-site attendant, instant prints, digital sharing (QR/AirDrop/SMS), custom overlays, props, and full setup and breakdown.' },
  { question: 'Can you customize the booth for our event?', answer: 'Absolutely. We design custom overlays, print templates, and start screens to match your exact theme, brand colors, or event aesthetic. We can also provide custom backdrops.' },
  { question: 'How quickly do guests get their photos?', answer: 'Instantly. Photos are printed on-site in seconds and digital copies are sent to guest phones via QR code, AirDrop, SMS, or email immediately.' },
  { question: 'Do you travel outside Chicago?', answer: '' },
  { question: 'What\'s the minimum rental duration?', answer: '2-hour minimum, with most events booking 3–4 hours. We offer flexible packages to match your event schedule and guest count. We recommend booking at least 6 months in advance as dates fill quickly.' },
]
