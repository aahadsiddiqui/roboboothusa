...
import {
  ...
  FiClock, FiX, FiGift, FiImage, FiStar, FiUsers, FiZap, FiShield, FiShare2
} from 'react-icons/fi'
...
/* ════════════════════════════════════════════════════════════════
   BIRTHDAYS & QUINCEAÑERAS — dedicated Chicago ad funnel.
   Public URL: /chicago/birthday-events (rewrites to /birthday-events).
   Distinct from pages/birthday.tsx, which is the Texas-only landing.
   ...
   ════════════════════════════════════════════════════════════════ */
export const getServerSideProps = getRegionalLandingSsp('/birthday-events')

export default function BirthdayEvents({ browserPath }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  const market = useMemo(() => getMarketForPath(browserPath), [browserPath])
  const publicPath = useMemo(
    () => (market.id === 'national' ? '/birthday-events' : `${market.basePath}/birthday-events`),
    [market.basePath, market.id],
  )
  const L = useCallback((s: string) => localizeMarketingCopy(s, market), [market])
  const areaPhrase = market.id === 'chicago' ? 'across Chicago & surrounding areas' : market.id === 'texas' ? 'across Texas' : 'across the USA'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fd.append('event-date', form.eventDate); fd.append('budget', form.budget); fd.append('event-type', 'Birthday / Quinceañera')
      ...
      fd.append('source', market.id === 'national' ? 'Birthday Funnel' : `Birthday Funnel (${market.analyticsRegion})`)
  ...
  return (
    <>
      <Head>
        <title>{L("Birthday & Quinceañera Robot Photobooth Chicago | Robo Booth")}</title>
        <meta
          name="description"
          content={L(
            "Chicago's first robot photobooth for birthday parties and quinceañeras. From 1st birthdays to Sweet 16s, quinces, and milestone celebrations — personalized overlays, table visits, and instant prints.",
          )}
        />
        <meta name="keywords" content="birthday photobooth Chicago, quinceañera photobooth Chicago, robot photobooth birthday Chicago, kids birthday photo booth Chicago, quinceañera photo booth" />
        <meta property="og:title" content={L("Chicago's First Robot Photobooth for Birthdays & Quinceañeras | Robo Booth")} />
        <meta property="og:description" content="The roaming robot photobooth that makes every birthday and quinceañera unforgettable. Custom themes, personalized prints, and white-glove service." />
        ...
        <div className="min-h-screen bg-black text-white overflow-x-hidden">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">
                <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
                  <div className="flex items-center gap-2 mb-3">
                    <FiGift className="w-4 h-4 text-[#fce4a6]" />
                    <div className="flex text-yellow-400 text-sm">★★★★★</div>
                    <span className="text-white/60 text-xs font-medium">5.0 Rating · Loved by Families {areaPhrase}</span>
                  </div>
          ...
          <section className="py-4 md:py-6 border-y border-[#fce4a6]/10 overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 mb-3">
              <p className="text-center text-[#fce4a6]/60 text-[10px] md:text-xs font-semibold tracking-[0.2em] uppercase">Birthdays, quinceañeras &amp; celebrations {areaPhrase}</p>
            </div>
            ...
            <div className="max-w-5xl mx-auto">
              <Reveal className="text-center mb-6">
                <h2 className="text-xl md:text-2xl lg:text-3xl font-black mb-1.5">
                  ...
                  <span className="text-[#fce4a6]">{areaPhrase}</span>
                </h2>
              </Reveal>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5 items-stretch">
                ...
                <Reveal>
                  <div className="relative rounded-3xl border border-white/20 bg-white/[0.04] p-6 md:p-7 h-full flex flex-col">
                    ...
                    <p className="text-white/50 text-xs text-center mb-6">The standalone birthday robot photobooth — fully set up, operated, and managed by our team.</p>
                ...
                <Reveal delay={0.1}>
                  <div className="relative rounded-3xl overflow-hidden border-2 border-[#fce4a6]/50 bg-gradient-to-br from-[#fce4a6]/10 via-black to-black p-6 md:p-7 shadow-2xl shadow-[#fce4a6]/10 h-full flex flex-col">
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      ...
                      <p className="text-white/60 text-xs text-center mb-6">Pair the roaming robot with a second photo experience — keep the energy high and the lines short all party long.</p>
                      <div className="space-y-2.5 mb-8 flex-1">
                        {[
                          'Robot Photobooth roaming table-to-table all party long',
                          'Add-on: Premium Photobooth or 360 Booth',
                          'Two photo experiences running simultaneously',
                          'Custom themed overlays with name, age & party theme',
                ...
                <Reveal delay={0.2}>
                  <div className="relative rounded-3xl overflow-hidden border-2 border-white/40 bg-gradient-to-br from-white/[0.08] via-black to-black p-6 md:p-7 h-full flex flex-col" style={{ boxShadow: '0 0 40px rgba(255,255,255,0.06)' }}>
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      ...
                      <p className="text-white/60 text-xs text-center mb-6">The ultimate celebration setup — multiple photo stations plus a photographer capturing every moment.</p>
                      <div className="space-y-2.5 mb-8 flex-1">
                        {[
                          'Everything in the Gold Package',
                          'Add-on: Premium Photobooth or 360 Booth',
                          'Professional event photographer covering key moments',
                          'Candid guest photography throughout the celebration',
                          'Professionally edited high-resolution gallery after the event',
          ...
          <section className="py-10 md:py-14 px-4 border-t border-white/5">
            <Reveal className="max-w-3xl mx-auto text-center">
              ...
              <p className="text-white/60 text-xs md:text-sm lg:text-base mb-5 max-w-lg mx-auto">
                {L('Join hundreds of families across the USA who trust Robo Booth to elevate their birthday parties, quinceañeras, and milestone celebrations. Personalized, fun, and fully managed — we handle everything.')}
              </p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/70 backdrop-blur-md p-0 md:p-4">
            <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 60 }}
              ...
              <h2 className="text-lg md:text-2xl font-black text-black mb-1 text-center">
                {packageType === 'gold' ? 'Book Gold Package' : packageType === 'bronze' ? 'Book Bronze Package' : packageType === 'platinum' ? 'Book Platinum Package' : 'Get a Birthday Quote'}
              </h2>

              <p className="text-black/60 text-xs md:text-sm mb-4 text-center">Tell us your celebration date and we&apos;ll confirm availability within 15 minutes.</p>
              {success ? (
...
/* ─── DATA ─── */
const howItWorks = [
  { title: 'You Book', desc: 'Tell us the celebration date, venue, theme, and guest count — whether it\'s a birthday, quinceañera, or milestone. We confirm availability fast and design a custom activation around your event.' },
...
const whyFamiliesLove = [
  { icon: <FiUsers className="w-5 h-5 md:w-6 md:h-6" />, title: 'Every Age Loves It', desc: 'From toddlers at a 1st birthday to grandparents at an 80th — the robot captivates every generation and brings the whole room together.' },
  { icon: <FiGift className="w-5 h-5 md:w-6 md:h-6" />, title: 'Table-to-Table Visits', desc: 'Our on-site attendant drives the robot throughout your venue, visiting every table so no guest gets left out of the fun.' },
  ...
  { icon: <FiImage className="w-5 h-5 md:w-6 md:h-6" />, title: 'Keepsake Prints', desc: 'Every guest walks away with a physical print in hand — a personalized souvenir they\'ll put on the fridge, not bury in a camera roll.' },
  { icon: <FiShield className="w-5 h-5 md:w-6 md:h-6" />, title: 'Zero Setup Burden', desc: 'No power outlets, no WiFi, no special venue requirements. Our team handles full setup and teardown — invisible to your party planning.' },
  ...
  { icon: <FiZap className="w-5 h-5 md:w-6 md:h-6" />, title: 'Fully Themed Experience', desc: 'Custom overlays with name, age, and theme — Winnie the Pooh, princess, quinceañera rose gold, casino, racing cars, and more. Every print celebrates the honoree.' },
  { icon: <FiCheck className="w-5 h-5 md:w-6 md:h-6" />, title: 'White-Glove Service', desc: 'A dedicated on-site attendant manages everything. You focus on the party — we handle the tech from start to finish.' },
]
...
const customizations = [
  ...
  { icon: <FiZap className="w-5 h-5" />, title: 'Custom Voice & Messaging', desc: 'Program the robot with celebration greetings — record a message from the honoree, their parents, or a loved one. The robot delivers it at every table for a personal touch guests never forget.' },
  { icon: <FiStar className="w-5 h-5" />, title: 'Any Venue, Any Size', desc: 'Banquet halls, backyard parties, restaurant private rooms, hotel ballrooms — we\'ve done them all. The robot adapts to your space and guest count seamlessly.' },
]
...
const faqs = [
  { question: 'How big is the Robot Photobooth?', answer: 'The robot is roughly the height of a standing adult and moves smoothly through normal event spaces. It needs no dedicated footprint — it roams table to table rather than occupying a corner of the room like a traditional booth.' },
  ...
  { question: 'Does the setup require WiFi or power from the venue?', answer: 'No. The robot is fully self-powered and self-connected. No cables, no outlets, no WiFi requirements — nothing for you or your venue to arrange.' },
  { question: 'How are photos shared with guests?', answer: 'Guests receive digital copies instantly to their phones via QR code, AirDrop, SMS, or email — and every guest also gets a physical print handed to them on the spot.' },
  { question: 'Is an attendant included?', answer: 'Yes. Every booking includes a dedicated on-site attendant who drives the robot, manages guest interactions, and handles setup and teardown. You never have to touch the technology.' },
