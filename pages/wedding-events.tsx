...
/* ════════════════════════════════════════════════════════════════
   WEDDINGS — dedicated Chicago ad funnel.
   Public URL: /chicago/wedding-events (rewrites to /wedding-events).
   Hybrid lockdown: no site nav in the header, but the footer stays so
   visitors can still reach contact + social. noindex.
   The single quote form is the only tracked form (Chicago form_submit).
   ════════════════════════════════════════════════════════════════ */
export const getServerSideProps = getRegionalLandingSsp('/wedding-events')

export default function WeddingEvents({ browserPath }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  const market = useMemo(() => getMarketForPath(browserPath), [browserPath])
  const publicPath = useMemo(
    () => (market.id === 'national' ? '/wedding-events' : `${market.basePath}/wedding-events`),
  ...
  return (
    <>
      <Head>
        <title>{L("Wedding Robot Photobooth Chicago | Chicago's First Robot Photobooth | Robo Booth")}</title>
        <meta
          name="description"
          content={L(
            "Chicago's first Robot Photobooth for weddings. It visits every table, delivers personalized photos to every guest, and prints keepsakes on the spot. Serving Chicago & surrounding areas.",
          )}
        />
        <meta name="keywords" content="wedding photobooth Chicago, robot photobooth wedding Chicago, wedding photo booth rental Chicago, wedding entertainment Chicago" />
        <meta property="og:title" content={L("Chicago's First Robot Photobooth for Weddings | Robo Booth")} />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-center">
                <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
                  ...
                  <h1 className="text-[1.65rem] leading-[1.15] md:text-4xl lg:text-5xl font-black md:leading-[1.1] mb-4">
                    ...
                    <span className="text-[#fce4a6]">Robot Photobooth for Weddings</span>
                  </h1>
